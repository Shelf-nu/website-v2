...
// Hosting facts, verified 2026-09-18 against product origin/main and live
// production (see PRODUCT-FACTS.md, "Hosting, data location and
// sub-processors"). Both claims below shipped on this site and in sales
// documents before they were caught.
const INFRA_RULES = [
  { re: /row[- ]level security|\bRLS\b/i,
    why: "RLS is enabled with zero policies. Workspace isolation is in application code (workspace-scoped queries plus a lint rule). Don't claim RLS or RLS policies." },
  { re: /frankfurt[^\n]{0,12}fly\.io|fly\.io[^\n]{0,12}frankfurt/i,
    why: "Fly.io runs the application servers in Amsterdam. Frankfurt (AWS eu-central-1) is where Supabase holds the data. Don't attribute Frankfurt to Fly.io." },
];
...
const TARGETS = [
  ...
  { roots: ["content"], exts: [".mdx", ".md", ".txt"], rules: [...MOBILE_LIVE_RULES, ...INFRA_RULES] },
  { roots: ["src"], exts: [".ts", ".tsx"], rules: [...MOBILE_LIVE_RULES, ...INFRA_RULES, ...SRC_ONLY_RULES, ...PRICE_RULES] },
];
