...
instead of only all at once. The booking becomes active (**Ongoing**) on the **first** item you check out, while everything you haven't reached yet stays reserved for the booking but available to verify.

The system works like a library checkout desk where books leave the counter as each patron collects them — the loan is open from the first item, and you keep a running tally of what's gone out and what's still waiting.
...
The headline above the bar adapts to the phase you're in. While any items are still **Booked**, it reads **"Check-out progress"** and counts how many items have left (checked out + returned). Once nothing is left to check out, it switches to **"Check-in progress"** and counts how many have come back. A legend underneath shows the exact count in each bucket.
