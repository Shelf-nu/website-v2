...
- **Managing bookings:** create, update and fulfil (check in/out) bookings, including those started by other users, and delete, archive or cancel any of them in bulk.
...
- **Bookings:** book assets for themselves and **check them out and back in on their own**, no admin needed. The **Custodian** field on a booking they create holds their own name and nobody else's, on the web and in the [Shelf Companion](/mobile-app) app alike. Their own bookings are also the limit of what they can act on in bulk: selecting rows and choosing Delete, Archive or Cancel touches only the bookings that belong to them, even when a workspace setting lets them see everyone else's. Taking items back off a booking stops at **Reserved**: a Self-service user can adjust their own reservation up to the point it goes out, but not after.
