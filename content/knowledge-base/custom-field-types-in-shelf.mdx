...
![Custom fields displayed on an asset detail page in Shelf](https://qliecghuzfchfjwaisyx.supabase.co/storage/v1/object/public/website-images/knowledgebase/custom-field-types-1.webp)
...
- **CSV Import**: declare the field type in the header (e.g. `cf:Purchase Date,type:date`); Shelf auto-creates the custom field if it does not exist, using the type from the header. For `option` fields, the set of allowed values is collected automatically from the data in that column.
