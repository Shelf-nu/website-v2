...
The server re-checks all three at the moment you reserve, so an edit someone else makes while you have the booking open cannot let an unreservable booking through. The first two come back word for word. The already-booked one is caught by the conflict check, which is more specific and names the assets that clash rather than repeating the tooltip. These checks now cover the [Shelf Companion app](/mobile-app) too, so the phone refuses exactly what the website refuses.
