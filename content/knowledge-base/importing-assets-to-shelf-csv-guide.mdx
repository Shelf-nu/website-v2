...
The two use different column names, so a file built for one is not a file for the other. This guide's headers are the importer's own keys (`title`, `valuation`, `assetModel`, `quantity`). The update workflow reads the labels a Standard export writes (`Name`, `Value`, `Asset model`, `Quantity`) and matches rows on `Asset ID` or `ID`.
...
1. Download the CSV template or barcode-enabled version from Shelf (or, if you're moving assets in from another Shelf workspace, use an **Import-ready** export instead: [Exporting Data from Shelf](/knowledge-base/export-exporting-workspace-data-from-shelf#choosing-an-export-format))
