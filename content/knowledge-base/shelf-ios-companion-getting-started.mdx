...
## 8. Bookings: Find, Build, Check Out and Check In

For workspaces using [Bookings](/features/bookings), the app now handles the field side of a booking end to end.
...
**Build out a booking by scanning.** Open a draft booking and tap **Scan to Add Assets**, then scan the assets and kits you're packing. Kits expand to their member assets automatically. You still create the booking itself (dates, custodian, name) in the web app; the app is where you fill it with gear from the floor.
...
Roles apply: self-service members act on their own bookings; admins and owners act on behalf of others.
...
- **Creating, editing, or rescheduling a booking from the app.** You can build a booking by scanning gear into it, check items out, and check them back in on mobile, but setting the booking up (dates, custodian, details) is still web-only. Mobile booking-create routes are tracked separately.
