...
seo:
  ...
  description: "How to download Shelf Companion from the App Store or Google Play, sign in with your existing Shelf account, and start scanning assets and kits, building bookings, running audits, and managing custody from your phone."
---
...
**The camera pauses itself, and any tap wakes it.** After about thirty seconds without a scan the camera stops and the screen reads **Camera Paused — Tap anywhere to resume scanning**. Tapping anywhere that is not a control brings it back. It used to take a tap on the small play button, because the strips across the top and bottom of the view swallowed everything else.

**Can't scan a label?** Tap **Enter code** to type a QR ID, barcode value, or sequential ID (a SAM, such as `SAM-0001`) by hand. The field opens in the strip at the top of the camera view with the keyboard already up, so one tap is enough to start typing — it used to sit at the bottom, where the keyboard covered it. When a drawer is open, the closed **Enter code** pill moves to the top strip as well, so it stays reachable. This is handy when a label is damaged, hard to reach, or printed on packaging the camera can't focus on. Typed SAM IDs resolve to their asset on both the general and audit scanners; the lookup itself needs no add-on (running a full audit still requires the [Audits add-on](/features/audits)). A barcode value shaped like a SAM ID — `LAPTOP-0001`, say — resolves too: the app checks your sequential IDs first and then your workspace's [barcodes](/knowledge-base/alternative-barcodes), so a label printed in that shape no longer ends in **Lookup Failed**. That second step is part of the alternative-barcodes add-on; without it, a SAM-shaped code that matches no asset still fails. It all runs on the server, so the app you already have picks it up with nothing to install.
...
- **Update the asset's location** — useful when you move an asset between rooms, buildings, vehicles, or job sites. For a [quantity-tracked](/knowledge-base/quantity-tracked-assets-and-consumables) asset this action is called **Placements** instead, because a pool can sit in several places at once (see below).
...
**The code your workspace labels things with.** The screen carries a **Codes** card that leads with the identifier your workspace chose in its code display preference — the Shelf QR, the SAM ID, or one of the asset's registered barcodes — rather than always leading with the QR. An asset that overrides the workspace choice for itself wins over it. When there is more than one code on the asset, the card's selector lets you switch between them, exactly as the web preview's dropdown does. If the preferred code cannot be shown because the asset does not carry one, the card says so in the server's own words, for example *"Your workspace prefers Code 128 but this item has no Code 128."* Asset list rows print the same resolved code under the name, so a row can be matched against a physical label without opening it. Kit screens follow the same rule. Registered barcodes need the [barcodes add-on](/knowledge-base/alternative-barcodes); without it the card holds the Shelf QR alone.

**Who is holding it, and through what.** When an individually tracked asset is out on a booking, the detail screen names the holder on an **In Custody Of** row, the booking on a **Via Booking** row, and when it left on a **Since** row — the phone's form of the web asset page's custody card. Tap the booking row to open it, when your role may open that booking; when it may not, the row still names the booking but does not tap through. Quantity-tracked assets do not get these rows, because a pool can be in several hands at once. Who sees them follows the workspace's [custody visibility](/knowledge-base/configure-what-self-service-and-base-users-can-see) rules, including the exception that lets a booking's own custodian see it.
...
**Who the custodian picker offers you.** The **Custodian** field on **New booking** and on a booking you edit holds the people your role may put on a booking, which is the same set the web form gives you. Administrators and Owners get the whole team, and can search it. Base and Self-service members get one name — their own — because a booking they make is a booking for themselves. The workspace's **custody visibility** setting does not change this: it governs whose custody you can *see*, never whom you may *assign*. See [Configure what Self-service and Base users can see](/knowledge-base/configure-what-self-service-and-base-users-can-see#what-the-toggles-never-reach).
...
**Kits stay together on the booking.** A booking that holds a kit shows the kit as **one row**, the same way the web booking page does. The row shows the kit's name and image, how many of its assets are on the booking, its category and location, and its status. Kit rows start closed: tap one to see the assets inside, and tap again to close it.

**A kit is picked as one thing.** When you **Select to Check Out**, **Select to Check In** or **Select to Remove**, the kit row gets a single tick box. Tap the row once and it picks every asset in the kit that can take that action (in check-in, only the ones still out). Tap it again to drop the kit. The assets inside a kit have no tick box of their own, and tapping one while you are selecting does nothing, so part of a kit can't go out, come back or leave the booking by accident. The web booking page works the same way. Starting a selection leaves your kits open or closed as they were, and the arrow at the end of a kit row still opens it, so you can look inside before you pick it. Assets that are not in a kit keep their own tick box.

A kit the mode cannot act on shows **no tick box at all**, the same way an asset in that state does. A kit that is already checked out has none while you **Select to Check Out**, and a kit that is not out yet has none while you **Select to Check In**. The row keeps its place in the list, so pictures, names, badges and arrows still line up down the column, and tapping the row or its arrow opens the kit as usual. **Select to Remove** keeps every tick box, because any kit on the booking can be taken off it. When you tick a kit, the frame drawn around its members is now closed on all four sides rather than open on the left.

The button at the bottom counts a kit as one item and says what it will act on, for example **Check Out 1 Kit & 1 Asset**, **Check In 3 Assets** or **Remove 2 Kits**. The question it asks and the message you get back count the same way: *"Check out 1 kit and 1 asset?"*, then *"1 kit and 1 asset checked out. The rest is still reserved."* They used to count the kit's members instead, so a three-asset kit plus one asset read *"Check out 4 selected assets?"* under a button that said 1 Kit & 1 Asset. This holds on the booking screen and in the scanner's check-out and check-in drawers. **Check Out All Assets** still names the booking's raw asset total, because it acts on the whole booking rather than on a selection. On the phone, an asset in a kit leaves a booking only together with its kit; to take off a single asset from a kit, use the web booking page. Removing a kit is sent as a kit removal when the booking holds every asset in it. If the booking holds only some of them, picking the kit removes the assets it does hold.
...
**Scan a kit into the fulfil scanner.** Scanning a kit's code on **Scan to assign & check out** used to come back with *"Kits can't be checked out from the reservation scanner"*. It works now: the kit goes on the booking whole, its individually tracked members assign outstanding reserved units of their model on the way, and the whole case goes out with the submit. A unit that belongs to a kit still cannot go out alone here — scan one without its kit and the answer is *"…" belongs to a kit, so it can't go out on its own. Scan the kit to take all of it, or scan another unit of the same model.* This was a server-side change, so **there is nothing to install**: every phone with Shelf Companion behaves this way, whatever version it is running. See [A unit inside a kit counts too](/knowledge-base/adding-assets-and-kits-to-a-booking#a-unit-inside-a-kit-counts-too).
...
**Find one item on a big booking.** Each asset on a booking shows where it is kept, on a location line under its name, the same way the web booking page does. Kit members show their own location too. Above the list, the **Search assets & kits** box narrows what you see as you type. It matches an asset's name, category and location, and the name, category and location of its kit. When one asset in a kit matches, the whole kit comes back, so you never see part of a kit. If nothing matches, the list says **No assets match**. Searching only changes what is on screen: anything you already selected stays selected, and the button still counts all of it. The phone's search does not look at SAM IDs, tags, QR codes or barcode values, and it treats commas as part of the text. Use the scanner to find an item by its label.

**What is still to go out sits at the top.** A booking's item list is ordered the way the web booking page orders it by default: everything still waiting to be checked out comes first, and everything already out sinks to the bottom. A kit moves as one block with its members kept together, and it only sinks once every member of it is out. A [quantity-tracked](/knowledge-base/quantity-tracked-assets-and-consumables) line stays at the top while any of its booked units are still waiting, and comes back to the top if units are returned. Items that tie break A→Z by name. That matters most halfway through a hand-over, when the two or three things still to collect stay where you are looking instead of being pushed under everything you have already loaded. The phone used to do the opposite and list the checked-out items first. This was a server-side change, so **there is nothing to install** — every phone with Shelf Companion already lists a booking this way, whatever version it is running.

**Check out one item at a time or in bulk.** You don't have to check out a whole booking at once (progressive check-out). A reserved booking offers three ways to hand gear over:

- **Check Out All Assets** checks out everything in one go. It is offered while the booking is **Reserved**.
- **Scan to Check Out** opens the scanner for this booking. Scan each asset or kit as you hand it over. The scanner only takes assets that are on this booking and still waiting to go out. Anything else is refused with the reason, for example **Not in This Booking**, **Already Checked Out** or **In Custody**. A kit's code adds its assets that can go out, and scanning the same kit again tells you **Already Covered**. Tap the button at the bottom, for example **Check Out 2 Assets**, to send the list. The scanner stays open while the booking still has gear to hand over, and takes you back to the booking once everything is out.
- **Select to Check Out** lets you tick the assets and kits you are handing over, then check them out together.

The first check-out moves the booking to **Ongoing**. After that, **Check Out All Assets** is gone, but **Scan to Check Out** and **Select to Check Out** stay on the booking until everything has gone out, so you can hand over the rest later. A lifecycle **progress bar** tracks each item through Reserved, Checked out, and Returned, so a half-packed booking reads accurately.

Scanning a [quantity-tracked](/knowledge-base/quantity-tracked-assets-and-consumables) asset checks out every unit of it that is still waiting on the booking. To take fewer units, use **Select to Check Out**, which asks how many.

**A reserved model you could not find does not hold the booking back.** A check-out needs one item to go out and nothing more. **Check Out All**, **Select to Check Out** and both scanners confirm the units still unassigned instead of refusing — the app names them the same way the web does, for example *"2 × Dell Latitude and 1 × HP LaserJet are not assigned yet. They stay on the booking so you can scan them later or release them."* Confirm and the booking goes Ongoing with what is ready; the reservations stay open, to be settled later with **Scan to assign & check out** or released from the web. A booking made only of reservations still cannot go out, because nothing would leave. See [Checking out while units are still unassigned](/knowledge-base/adding-assets-and-kits-to-a-booking#checking-out-while-units-are-still-unassigned).

Scan to Check Out and Select to Check Out follow the same rules as the server. Administrators and Owners see them on any booking. Self-service members see them only on bookings they created or are the custodian of. Base members can't check gear out, so they see neither.
...
**A sheet you typed into stays open if the save fails.** The sheets that take numbers — adjust quantity, the check-in disposition split, and Manage Placements — used to close the moment you tapped save, so a save the server refused took your numbers with it and you started again from memory. They now stay open with the error and everything you entered still in place, ready to correct and send again.

**Times you pick are your account's times.** When you set a booking's start and end on the phone, the date and time pickers work in the time zone from your Shelf [date and time preferences](/knowledge-base/date-time-and-timezone-preferences), not the zone your handset happens to be in. Pick 9:00 and the booking reads 9:00 afterwards, wherever you are standing. Before this, anyone whose phone zone differed from their Shelf preference picked one time and saw another the moment they saved. On iPhone the inline date picker also follows the app's own **Settings ▸ Appearance** rather than the phone's, so running the app in light mode on a dark phone (or the other way round) no longer draws the calendar dark-on-dark or white-on-white. Android was already readable and is unchanged.
...
- **Barcodes add-on** — The app's scanners read every code type Shelf registers: QR, Code 128, Code 39, DataMatrix and EAN-13, on both the general scanner and the audit scanner. Scanning any registered code lands on its asset. Since the update of 21 September 2026 an asset's and a kit's detail screens also **list** the codes it carries, in a **Codes** card that leads with the one your workspace prefers and lets you switch between the rest. Adding and editing codes is still a web job. See [Alternative barcodes](/knowledge-base/alternative-barcodes).
...
- **Adding or editing an asset's codes.** The detail screen lists the codes an asset carries, but registering a new barcode or changing one is a web job.

This guide describes **version 1.5.0**, live on the App Store and Google Play since 31 August 2026. Grouping a booking's kits and the **Manage Placements** editor reached 1.5.0 through an over-the-air update on 10 September 2026, so the version number did not change. **Scan to Check Out**, picking a kit as one thing, and the booking's location lines and search followed the same way on 14 September 2026. A further update on 21 September 2026 brought the **Codes** card on asset and kit screens, the custody-through-a-booking rows, the paused camera resuming on any tap, manual code entry above the keyboard, kits counted as one thing in the check-out and check-in questions, sheets that stay open when a save fails, and the tick box hidden on a kit the mode cannot pick. The app downloads an update like this in the background when you open it and uses it from the next time you open it. If your app is older, take the update from the [App Store](https://apps.apple.com/app/id6765639874) or [Google Play](https://play.google.com/store/apps/details?id=com.shelf.companion). Two items that used to sit on the list above shipped in 1.3.0 and moved into the guide: **recording where a scan happened** (step 4) and **claiming a brand new QR code** (step 4, for admins and owners). The booking calendar, the audit evidence action, and the model and asset ID rows need **1.4.0**. [Connecting to your own server](/knowledge-base/connect-shelf-companion-to-your-own-server), undoing an audit scan, and the scanner's clearer results need **1.5.0**.
...
- [Shelf Companion overview](/mobile-app) — product page with screenshots, features, and the App Store and Google Play links.
