...
The button at the bottom counts a kit as one item and says what it will act on, for example **Check Out 1 Kit & 1 Asset**, **Check In 3 Assets** or **Remove 2 Kits**. On the phone, an asset in a kit leaves a booking only together with its kit; to take off a single asset from a kit, use the web booking page. Removing a kit is sent as a kit removal when the booking holds every asset in it. If the booking holds only some of them, picking the kit removes the assets it does hold.
...
Scanning a [quantity-tracked](/knowledge-base/quantity-tracked-assets-and-consumables) asset checks out every unit of it that is still waiting on the booking. To take fewer units, use **Select to Check Out**, which asks how many. A booking that still has model reservations waiting for a physical unit has to be fulfilled first with **Scan to assign & check out** (see above).
