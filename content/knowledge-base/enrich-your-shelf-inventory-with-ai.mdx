...
```
I'm uploading a Shelf asset management CSV. Many cells are empty
and I need help filling them in.

For each row, based on the asset Name and Category, please:
- Fill empty "Tags" cells with up to 3 relevant comma-separated
  tags. If tags already exist, append new ones after a comma.
- Fill empty "Value" cells with a reasonable estimated value
  in the same currency format as existing rows. ONLY if you can
  cite a basis — otherwise leave blank.
- Fill empty custom field cells where you can make a reasonable
  inference from the asset name (e.g. "Manufacturer", "brand",
  "model", "condition").

IMPORTANT RULES:
- Do NOT change any existing values — only fill blank cells.
...
- If you're not confident about a value, leave the cell blank.
- Return the complete CSV as a downloadable file.
```
