...
Two format details decide whether your file works as an update:

- **Pick Standard, not Import-ready.** The update workflow matches rows on `Asset ID` or `ID`, and Import-ready files carry neither: they use the importer's own lowercase headers to create assets rather than edit them.
- **Quantity needs a manual pass.** A Standard export writes a pooled asset's quantity with its unit attached, as `10 boxes`, and the update workflow needs the number alone. `Min quantity`, `Unit of measure`, and `Consumption type` are not exported at all, so add those columns yourself, spelled exactly that way, when you want to change them in bulk.
