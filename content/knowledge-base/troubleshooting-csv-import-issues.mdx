...
In **Update Existing Assets**, an unrecognised or inapplicable column never stops the file. It is listed in the preview and skipped, so the usual symptom is that everything else updated and one column did nothing. The common causes:

- **The header is spelled differently from the export label.** Headers are matched exactly against what a Standard export writes. The value column is `Value`, not `Valuation`. A header Shelf doesn't recognise is listed under unrecognized columns and ignored.
