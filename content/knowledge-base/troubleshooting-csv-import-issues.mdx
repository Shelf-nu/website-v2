...
This one is about the **Import** flow that creates new assets. It takes a file of any size, but very large CSVs (10,000+ rows) may time out on slower connections. If this happens:

- Split the file into batches of 2,000 to 5,000 rows
- Import each batch separately
- All batches land in the same workspace

**Update existing is different.** It stops at **1,000 assets per file** whatever your connection, because each row can trigger several database calls. Split for that flow at 1,000, not at 2,000.
