...
|---|---|---|---|
| **Location** | Where is this asset right now? | One (quantity-tracked assets can split units across several) | Studio B, Meander 901, Truck 4, Server Room |
...
A location is a **physical place**. Shelf supports a [hierarchy up to 12 levels deep](/knowledge-base/sublocations-organize-locations-into-hierarchies) (building → floor → room → shelf). An individually tracked asset is in exactly **one location** at a time; a [quantity-tracked asset](/knowledge-base/quantity-tracked-assets-and-consumables) can have its units split across several, since a pool of 100 cables might sit in two stockrooms at once.
...
Locations also work at the [kit](/knowledge-base/kits#kit-location) level: give a kit a location and its member assets move with it, so gear that travels together is relocated in one action.
