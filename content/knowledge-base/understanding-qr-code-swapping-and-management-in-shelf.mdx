...
- **An asset holds one QR code at a time.** Linking a new one replaces the old, and the old sticker goes back to your workspace's pool. An asset can therefore have carried several codes over its life, but never two at once — so if you have printed several stickers for the same item, only the one currently linked resolves to it, and the rest are unlinked codes you can put on anything
