...
- **An asset holds one Shelf QR code at a time.** Linking a new Shelf code replaces the old one, and the old sticker goes back to your workspace's pool. So if you printed several Shelf stickers for the same item, only the one currently linked resolves to it, and the rest are unlinked codes you can put on anything. This applies only to the QR codes Shelf issues: with the [Alternative Barcodes](/knowledge-base/alternative-barcodes) add-on, the same asset can also carry extra codes, including External QR codes from your own system, and adding them never replaces its Shelf code
