...
- The **asset page**, where the blue **In custody** badge sits beside the status and the **Custody** card on the overview names the holder. For an asset checked out on a booking, the same card reads **In custody of** the booking's custodian **via** the booking, **Since** the booking's start.
...
The batch scanner already reached part of this. Its blocker list names **individually tracked assets that are part of a kit** for both **Assign Custody** and **Release Custody**, and it always has. That check reads kit membership rather than where the hold came from, so it was never the same rule, and it was the only place the rule existed. The refusal above is what the asset page, the bulk index action and the app were missing.

Quantity-tracked pools work differently and are unaffected. A pool can be held by several custodians at once and by a kit at the same time, so **Assign** and **Release Quantity Custody** act on one custodian's slice by name. Releasing your own slice never touches the units the kit is holding, and the kit's slice still clears only through the kit. The batch scanner follows the same split: a pool whose only held units came with its kit is listed as **held through a kit**, with the advice to scan the kit's QR instead.
