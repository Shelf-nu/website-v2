...
![Assets index with 20 assets selected and the Actions menu open — "Download QR Codes" is the top action in the dropdown](https://qliecghuzfchfjwaisyx.supabase.co/storage/v1/object/public/website-images/knowledgebase/avery-bulk-download-qr.jpg)
...
![Examples of custom Avery-compatible label designs built around Shelf unclaimed codes — different shapes, colors, and branded layouts](https://qliecghuzfchfjwaisyx.supabase.co/storage/v1/object/public/website-images/knowledgebase/avery-custom-label-designs.webp)
...
Unclaimed codes are a **Plus and Team plan** feature — they can't be requested on the Free plan. Batches are delivered through Shelf support; it isn't a self-serve button in the app today:

1. **Contact Shelf support** (in-app chat or [hello@shelf.nu](mailto:hello@shelf.nu)) with the quantity you need. Typical batches run from 50 to 1,000 codes.
2. **Receive a ZIP file** from the team — one per batch. The ZIP contains the codes as individual **SVG files**, one per code, with filenames like `20260416 - aofj28an3x.svg`. SVG is the format most design tools prefer for label work because it stays sharp at any print size.
3. **Import the SVGs** into your design tool of choice (Avery Design & Print, Canva, Illustrator, Figma, InDesign — anything that accepts SVG).
...
Or skip the DIY design step and order ready-made Shelf-branded labels at [store.shelf.nu](https://store.shelf.nu/?ref=kb_avery_article).
...
**Why it matters:** labels get scratched, torn, or water-damaged. If the QR code is physically unreadable, the printed text is your recovery path. Asset names, Asset IDs (SAM-0042), and QR IDs are all searchable in Shelf — type any of them into Quick Find (⌘K) or the search bar, find the asset in seconds, then [relink a fresh QR code](/knowledge-base/understanding-qr-code-swapping-and-management-in-shelf) to it. No data lost, no asset orphaned.

If you want a different layout — swap QR ID for Asset ID, add your own logo, change the font — use the [asset-label-designer tool](/tools/asset-label-designer), or switch the ID display as described in [Asset Identifiers](/knowledge-base/asset-identifiers-qr-id-sam-id-property-id).

### What about DYMO, Brother, or Zebra label printers?

Those are roll-fed thermal printers — different workflow. Shelf supports them directly in the app via the asset page Print button. To choose which ID prints under the code, see [Asset Identifiers](/knowledge-base/asset-identifiers-qr-id-sam-id-property-id).
...
- [Asset Identifiers: QR ID, SAM ID, and Your Own Reference Numbers](/knowledge-base/asset-identifiers-qr-id-sam-id-property-id) — choose what prints on your labels and how assets are numbered
