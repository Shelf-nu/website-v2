...
Recipients are deduplicated before sending. If someone appears in multiple layers (they are both the custodian and on the always-notify list), they receive one email, not two. The person performing an action is excluded from the immediate notification — they already know what they did — with two deliberate exceptions: the custodian and, while **Notify booking creator** is on, the creator, both of whom are on the list precisely to hold a complete record of the booking. Scheduled reminders (checkout, check-in, overdue) reach everyone regardless.
