...
Recipients are deduplicated — no one receives the same email twice even if they appear in multiple layers. The person performing an action is excluded from the immediate notification (they already know what they did), except when they are the custodian or the creator — those two are on the list to keep a complete record and are never dropped. Scheduled reminders still reach everyone.
