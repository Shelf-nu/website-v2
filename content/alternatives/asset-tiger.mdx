...
Shelf provides flexible custom fields across all plans and a built-in [Reports area](/features/reports) with ready-to-run reports designed around the operational questions: Booking Compliance (on-time vs late), Top Booked Assets, Top Booked Kits, Monthly Booking Trends, Overdue Items, Asset Inventory, Asset Activity Summary, Asset Utilization, Idle Assets, Asset Distribution, and Custody Snapshot. Each report supports timeframe and filter controls; every report exports to CSV and chart-based reports also export to PDF.
