...
| **Mobile experience** | Native iOS/Android app | Native iOS app (Shelf Companion) + mobile web; Android in development |
...
Shelf uses QR codes as the primary operational interface. Scan to check out, scan to return, scan to update location, scan to see the full custody history. Every smartphone becomes a scanner in the browser, and the free [Shelf Companion app](/mobile-app) on iOS gives you a native-feeling field experience when you want one (Android in development). For teams doing equipment handoffs on loading docks, at event venues, or in campus gear rooms, the difference between "scan to find" and "scan to do" is substantial.
...
- **Native mobile on both iOS and Android**: Sortly ships a native app on both platforms today. Shelf has Companion on iOS, with Android in development
