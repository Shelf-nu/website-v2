...
| **Consumables / stock levels** | Yes — gated to the Professional tier as an add-on | On every plan — quantity, unit of measure, min-quantity low-stock alerts |
...
| **Operational reports** | Asset and depreciation reports with exports | Ten built-in operational reports (booking compliance, utilization, idle, custody, distribution) with CSV/PDF export |
...
Timly’s pricing starts with the Essential+ tier, but teams needing GPS tracking, API access, or consumables (warehouse) management must upgrade to the higher Professional tier as an optional add-on. Enterprise pricing is not published. Feature gating across tiers means the platform you evaluate during a trial may not reflect what you actually pay for.
...
Shelf is designed mobile-first. QR scanning, custody transfers, and booking confirmations all work in any smartphone browser. For an iPhone-native field experience, the free [Shelf Companion app](/mobile-app) is on the App Store and Google Play. Either path, the workspace, login, and data are the same as the web.
