...
| **Consumables / stock levels** | Yes — only on the higher Professional tier | On every plan — quantity, unit of measure, min-quantity low-stock alerts |
...
| **Mobile experience** | Web-based, reviewers note room for improvement | Native iOS & Android app (Shelf Companion) + mobile web |
| **Multi-team workspaces** | Single inventory with roles | Separated workspaces per department |
| **Operational reports** | Asset and depreciation reports with exports | Built-in operational reports (booking compliance, top booked assets and kits, utilization, idle, custody, distribution) with CSV/PDF export |
...
Timly’s pricing starts with the Essential+ tier, but teams needing GPS tracking, API access, or consumables management must upgrade to the higher-priced Professional tier. Enterprise pricing is not published. Feature gating across tiers means the platform you evaluate during a trial may not reflect what you actually pay for.

Shelf is open source with transparent pricing. Teams can self-host for free or use the managed cloud service, and consumables come standard: every plan can [track supplies by quantity](/features/consumables-tracking) with a unit of measure and min-quantity low-stock alerts—no upgrade required. There are no features locked behind opaque enterprise tiers, and no vendor lock-in—your data and your deployment are yours.
...
Shelf is designed mobile-first. QR scanning, custody transfers, and booking confirmations all work in any smartphone browser. For a native field experience on iPhone and Android, the free [Shelf Companion app](/mobile-app) is on the App Store and Google Play. Either path, the workspace, login, and data are the same as the web.
