...
| **Unlimited users** | Yes (all tiers) | Yes, on Team |
| **ERP integrations** | SAP, Oracle, Dynamics, Intune | Not a current focus — open source codebase and CSV import/export |
| **Mobile experience** | Native app (reviewers note performance issues) | Native iOS & Android app (Shelf Companion) + mobile web |
| **Multi-team workspaces** | Single inventory with role-based access | Separated workspaces per department |
| **Operational reports** | Comprehensive register, depreciation, and audit reports | Built-in operational reports (booking compliance, top booked assets and kits, utilization, idle, custody, distribution) with CSV/PDF export |
...
Shelf is designed mobile-first. QR scanning, custody transfers, and booking confirmations all work in any smartphone browser, and the free [Shelf Companion app](/mobile-app) is on the App Store and Google Play for a native experience on iPhone and Android. Either path, there are no app store updates to manage on the web side, and the phone gets the same feature set as the desktop rather than a cut-down one.

See: [Asset Tracking](/solutions/asset-tracking)

![Asset list showing tracked equipment inventory](https://qliecghuzfchfjwaisyx.supabase.co/storage/v1/object/public/website-images/features/search-hero.webp)
