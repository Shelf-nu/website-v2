...
| **Operational reports** | Asset and assignment reports with CSV export | Built-in operational reports (booking compliance, top booked assets and kits, utilization, idle, custody, distribution) with CSV/PDF export |
...
Shelf maintains a permanent, timestamped [custody chain](/features/custody) for every asset. Every check-out, return, and transfer is recorded with the user's identity, location, and timestamp. Need proof of who had a projector last Thursday? Filter by asset and date, then export the trail as a record.
