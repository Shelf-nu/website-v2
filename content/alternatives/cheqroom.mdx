...
### 7. Granular Booking Notification Control

In Cheqroom, booking notifications follow fixed rules tied to roles and reservation status. There is no mechanism to reduce notification noise for admins who do not manage every booking, add a specific person to a single checkout, or designate an operations lead who should receive everything.

Shelf's booking notification system is fully configurable. Workspace admins set defaults in **Settings > Bookings**: toggle whether booking creators are automatically notified, control whether all admins receive a broadcast on new reservations, and build an "always notify" list for people who need complete visibility. On individual bookings, anyone can add per-booking recipients — the department head who needs to know about one specific high-value checkout, for instance.

The result is less inbox noise for uninvolved admins and better coverage for the people who actually care about each booking.
