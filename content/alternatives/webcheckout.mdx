...
WebCheckout ships an extensive set of checkout, allocation, and equipment-history reports tuned for academic media-center operations. Shelf's [Reports area](/features/reports) takes a lighter, self-serve approach: ready-to-run operational reports (Booking Compliance, Top Booked Assets, Top Booked Kits, Monthly Booking Trends, Overdue Items, Asset Inventory, Asset Activity Summary, Asset Utilization, Idle Assets, Asset Distribution, Custody Snapshot) with timeframe and filter controls, available on every plan. Every report exports to CSV; chart-based reports also export to PDF.
