...
[Asset Reminders](/features/asset-reminders) let you set a reminder for any maintenance event:
...
Each reminder is attached to a specific asset, is set for a specific date, and notifies designated team members by email as the date approaches. When the work is done, log it and set the next reminder.

![Reminders tab on an asset showing calibration, cleaning, warranty, and disposal reminders with per-reminder status](https://qliecghuzfchfjwaisyx.supabase.co/storage/v1/object/public/website-images/features/asset-reminders.webp)
...
### Spare Parts and Consumables — Basic Counts

Maintenance runs on supplies: filters, lubricants, gaskets, fasteners, cleaning materials. Shelf lets you keep a simple count of these by [tracking them by quantity](/features/consumables-tracking) — one record per type with a running total, a unit of measure, and a minimum-stock threshold. When the count reaches or falls below that threshold, the workspace owner and every admin get a low-stock email so the part gets reordered before the next service is blocked.

This is a stock count with alerts, not a procurement system. Shelf does not generate purchase orders or link parts consumption to work orders — that is CMMS territory. For most teams, knowing "we're low on filters" in time to reorder is exactly enough.

See: [Track consumables by quantity](/features/consumables-tracking)

---

## Key Maintenance Features

- **Date-based reminders**: Set email notifications ahead of any due date; add the next one after each service
...
| **Spare-parts stock counts** | Basic — count by quantity with low-stock email | Yes — full parts catalog + purchase orders |
...
4. **Set up asset reminders** — add reminders ahead of inspection, calibration, and preventive-maintenance due dates.
...
Yes. Each asset gets its own reminders, set individually. Add a reminder for the forklift's next safety check and another for the fire extinguisher's next inspection — each on its own date.
