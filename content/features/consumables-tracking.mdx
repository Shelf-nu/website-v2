...
![Illustration: an asset list showing individually-tracked items, each with its own QR ID, alongside a quantity-tracked row for a pool of identical units with a count and a low-stock threshold.](/images/features/consumables/tracking-methods.svg)

_Illustrative — these graphics represent the Shelf interface, not screenshots._
...
![Illustration: a "Used up" item card (consumed, not returned) beside a "Returnable" item card whose check-in records what returned versus what was used.](/images/features/consumables/consumption-types.svg)
...
![Illustration: a custody panel for a pool of 100 USB-C cables — a segmented availability bar shows two people each holding a portion (Sarah 20, Mike 15) with 65 available, plus per-person Release actions and an Assign button.](/images/features/consumables/quantity-custody.svg)
...
![Illustration: an asset row flagged "Low stock" (40 left, minimum 50) and the resulting alert — emailed to the workspace owner and shown in-app to whoever made the change.](/images/features/consumables/low-stock-alert.svg)
