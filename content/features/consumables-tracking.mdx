...
- **Bookings**: reserve N units from the pool for a date range, and adjust the booked quantity as plans change. Availability is calculated for those dates, so bookings in different weeks can each draw on the same stock, while a genuine overlap that exceeds the pool is flagged with a **Stock conflict** pill on the bookings list and an **Insufficient stock** badge on that asset's row inside the booking.
