...
Shelf is developing a companion mobile app for iOS that provides barcode and QR code scanning for asset identification. The mobile app communicates with the same Shelf API as the web platform — there is no separate backend. The following disclosures apply specifically to the mobile app:

**Camera access** — The app requests access to your device camera to scan QR codes and barcodes for asset identification. The live camera feed used for scanning is processed on the device and is not itself recorded or stored. Separately, the app lets you attach images to your data, such as asset photos, audit-evidence photos, and profile pictures. These images are uploaded to and stored by Shelf via the same Shelf API as the web platform, linked to your account, and used only to provide the service. They are not used for tracking or advertising.
